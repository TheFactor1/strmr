...
const TMDBSettingsScreen = () => {
	...
	const loadSettings = async () => {
		...
		try {
			...
			logger.log(
				'[TMDBSettingsScreen] API key status:',
				savedKey ? 'Found' : 'Not found',
			);
			logger.log(
				'[TMDBSettingsScreen] Use custom API setting:',
				savedUseCustomKey,
			);
	...
	const saveApiKey = async () => {
		...
		try {
			...
			if (!trimmedKey) {
				...
				setTestResult({
					success: false,
					message: t('tmdb_settings.empty_api_key'),
				});
				return;
			}
			...
			if (await testApiKey(trimmedKey)) {
				...
				setTestResult({
					success: false,
					message: t('tmdb_settings.invalid_api_key'),
				});
			}
		} catch (error) {
	...
	const toggleUseCustomKey = async (value: boolean) => {
		...
		try {
			...
			logger.error(
				'[TMDBSettingsScreen] Failed to toggle custom key setting:',
				error,
			);
		}
	};
	...
	const fetchExampleLogos = async (show: (typeof EXAMPLE_SHOWS)[0]) => {
		...
		try {
			...
			logger.log(
				`[TMDBSettingsScreen] Fetching ${show.name} with TMDB ID: ${tmdbId}`,
			);
			...
			logger.error(
				`[TMDBSettingsScreen] Error fetching ${show.name} preview:`,
				err,
			);
		} finally {
	...
	useEffect(() => {
		const loadSelectedShow = async () => {
			try {
				const savedShowId = await mmkvStorage.getItem(
					'tmdb_settings_selected_show',
				);
				if (savedShowId) {
					const foundShow = EXAMPLE_SHOWS.find(
						(show) => show.imdbId === savedShowId,
					);
					if (foundShow) {
			...
			>
					...
					{settings.enrichMetadataWithTMDB && (
						<>
							...
							<Text
								style={[
									styles.settingTitle,
									{ color: currentTheme.colors.text, marginBottom: 4 },
								]}
							>
							...
							<Text
								style={[
									styles.settingDescription,
									{ color: currentTheme.colors.mediumEmphasis, marginBottom: 16 },
								]}
							>
							...
							<View style={styles.settingRow}>
								<View style={styles.settingTextContainer}>
									<Text
										style={[styles.settingTitle, { color: currentTheme.colors.text }]}
									>
										{t('tmdb_settings.production_info')}
									</Text>
									<Text
										style={[
											styles.settingDescription,
							...
							<View style={[styles.settingRow, { marginBottom: 0 }]}>
								<View style={styles.settingTextContainer}>
									<Text
										style={[styles.settingTitle, { color: currentTheme.colors.text }]}
									>
										{t('tmdb_settings.movie_collections')}
									</Text>
									<Text
										style={[
											styles.settingDescription,
				...
				>
					<View style={styles.sectionHeader}>
						...
						<Text style={[styles.sectionTitle, { color: currentTheme.colors.text }]}>
							{t('tmdb_settings.api_configuration')}
						</Text>
					</View>
					<Text
						style={[
					...
					<View style={styles.settingRow}>
						<View style={styles.settingTextContainer}>
							<Text style={[styles.settingTitle, { color: currentTheme.colors.text }]}>
								{t('tmdb_settings.custom_api_key')}
							</Text>
							<Text
								style={[
									styles.settingDescription,
						...
						<>
							...
							<View style={styles.statusRow}>
								<MaterialIcons
									...
									size={20}
									color={
										isKeySet ? currentTheme.colors.success : currentTheme.colors.warning
									}
								/>
								<Text
									style={[
								...
								>
									{isKeySet
										? t('tmdb_settings.custom_key_active')
										: t('tmdb_settings.api_key_required')}
								</Text>
							</View>
							...
							<View style={styles.apiKeyContainer}>
								...
								<View style={styles.buttonRow}>
									...
									{isKeySet && (
										...
										>
											<Text
												style={[styles.buttonText, { color: currentTheme.colors.error }]}
											>
												{t('common.clear')}
											</Text>
										</TouchableOpacity>
								...
								<TouchableOpacity style={styles.helpLink} onPress={openTMDBWebsite}>
									<MaterialIcons
										name="help"
										size={16}
										color={currentTheme.colors.primary}
									...
									/>
									<Text
										style={[styles.helpText, { color: currentTheme.colors.primary }]}
									>
					...
					>
						<View style={{ flexDirection: 'row', alignItems: 'center' }}>
							...
							<Text
								style={[
									styles.buttonText,
									{ color: currentTheme.colors.white, marginLeft: 8 },
								]}
							>
								{t('tmdb_settings.clear_cache')}
							</Text>
						</View>
					</TouchableOpacity>
				...
				<View style={styles.attributionContainer}>
					...
					<View style={{ width: '90%' }}>
						...
						>
							{t('tmdb_settings.disclaimer_part_one')}
						</Text>
						<Text
							style={[
								styles.attributionText,
						...
						>
							{t('tmdb_settings.disclaimer_part_two')}
						</Text>
					</View>
				</View>
							...
							<TouchableWithoutFeedback>
								...
								>
									...
									<View style={styles.modalHeader}>
										...
										>
											{t('tmdb_settings.choose_language')}
										</Text>
										<Text
											style={[
										...
										>
											{t('tmdb_settings.choose_language_desc')}
										</Text>
									</View>
									...
									<View style={styles.searchSection}>
										...
										>
											...
											/>
											<TextInput
												placeholder={t('tmdb_settings.search_language_placeholder')}
												placeholderTextColor={currentTheme.colors.mediumEmphasis}
									...
									<View style={styles.languagesSection}>
										...
										>
											{languageSearch.length > 0
												? t('tmdb_settings.search_results')
												: t('tmdb_settings.all_languages')}
										</Text>
