...
export const useWatchProgress = (
	...
	const [watchProgress, setWatchProgress] = useState<WatchProgressData | null>(
		null,
	);
		...
		(
			...
			if (parts.length === 3) {
				...
				const episode = currentEpisodes.find(
					(ep) =>
						ep.season_number === parseInt(seasonNum) &&
			...
			const episodeByStremioId = currentEpisodes.find(
				(ep) => ep.stremioId === episodeId,
			);
					...
					if (episodeId) {
						...
						if (incompleteProgresses.length > 0) {
							...
							if (watchedEpisodeNumbers.length > 0) {
								const lastWatched =
									watchedEpisodeNumbers[watchedEpisodeNumbers.length - 1]!;
								const currentEpisodes = episodesRef.current;
